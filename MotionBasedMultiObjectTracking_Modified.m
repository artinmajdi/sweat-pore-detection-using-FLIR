...
function MotionBasedMultiObjectTracking_Modified()
...
for index = 1:L
    ...
    [frame, Background] = removeBackground(frame);
    [mask, ~ , ~] = apply_filter_V2(frame, Background, "same");
    mask(:,1:60) = 0;
    mask = bwareafilt(mask,[5,1e8]);
    objects = regionprops(mask , 'Area' , 'BoundingBox' , 'Centroid');
    centroids = cat(1,objects.Centroid);
    bboxes = int32(cat(1,objects.BoundingBox));
...
%         obj.reader = vision.VideoFileReader('atrium.mp4');
        obj.reader = vision.VideoFileReader('G:\FLIR Datasets\Dataset\new_Jan2\Rec-000020 - Copy - test.wmv');
